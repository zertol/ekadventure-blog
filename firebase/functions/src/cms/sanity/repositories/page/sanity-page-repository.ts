...
export class SanityPageRepository extends SharedRepository implements IPageRepository {
    async fetchPrivacyPolicyPage(): Promise<PageType> {
        const response = await fetch(`${formatString(Constants.SANITY_BASE_URL, process.env.SANITY_PROJECT_ID)}/query/production?query=${encodeURIComponent(PRIVACY_PAGE_QUERY)}`);
